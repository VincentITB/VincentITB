...
const TEXT_EXTENSIONS = ['txt', 'md', 'json', 'js', 'ts', 'css', 'html', 'htm', 'xml', 'csv',
  'py', 'java', 'c', 'cpp', 'h', 'rb', 'go', 'rs', 'sh', 'yml', 'yaml', 'log', 'ini', 'sql'];
...
async function openViewer(entry) {
  ...
  viewerIcon.textContent = iconForFile(entry.name, entry.mimeType);
  viewerMeta.textContent = `${formatSize(entry.size)} · ${entry.mimeType || extOf(entry.name).toUpperCase() || 'archivo'} · añadido el ${new Date(entry.dateAdded).toLocaleDateString('es-ES')}`;

  viewerBody.innerHTML = '<p class="viewer__unsupported">Cargando vista previa…</p>';
...
async function renderPreview(entry) {
  ...
  const unsupported = (msg) => {
    ...
    wrap.className = 'viewer__unsupported';
    wrap.innerHTML = `<div class="item__icon">${iconForFile(entry.name, mime)}</div><p>${msg}</p>`;
  ...
  try {
    ...
    } else if (mime.startsWith('text/') || TEXT_EXTENSIONS.includes(ext)) {
      const text = await entry.blob.text();
      const pre = document.createElement('pre');
      pre.textContent = text.length > 300000
        ? text.slice(0, 300000) + '\n\n… (archivo truncado en la vista previa, descárgalo para verlo completo)'
        : text;
      viewerBody.appendChild(pre);

    } else {
      unsupported('No hay vista previa disponible para este tipo de archivo. Descárgalo para abrirlo.');
    }
  } catch (err) {
    console.error(err);
    unsupported('No se pudo generar la vista previa de este archivo.');
  }
}
